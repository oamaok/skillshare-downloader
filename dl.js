...
const cleanName = name => name.toLowerCase().replace(/[^a-z0-9]+/ig, ' ').trim().replace(/ /ig, '_')
...
async function getVideoInfo(accountId, policyKey, session) {
  ...
  const response = await request(url, { headers: {Accept:"application/json;pk="+policyKey } })
  return JSON.parse(response)
}

async function getPolicyKey(skillshareResponse) {
  const [url] = skillshareResponse.toString().match(/players\.brightcove\.net\/[^"]+\/index\.min\.js/);
...
function download(url, file) {
  return new Promise((resolve, reject) => {
    req(url).pipe(fs.createWriteStream(file)).on('finish', resolve)
  })
}
...
(async () => {
  ...
  const skillshare = JSON.parse(skillshareResponse.toString().match(/SS\.serverBootstrap = (.+);\n/)[1])
  const { brightcoveAccountId, shareLinks } = skillshare.pageData.videoPlayerData;
  ...
  const videoInfos = (await Promise.all(sessions.map(session => getVideoInfo(brightcoveAccountId, policyKey, session)))).filter(Boolean)

  for (let sessionId in sessions) {
    const session = sessions[sessionId]
    const filename = path.join(folderName, (session.rank + 1) + ' - ' + cleanName(session.title) + '.mp4')
    ...
    const videoUrl = videoInfo.sources
      .filter(source => (source.src || '').startsWith('https') && source.container === 'MP4')
      .sort((a, b) => b.avg_bitrate - a.avg_bitrate)[0]
      .src
